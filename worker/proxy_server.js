...
let rememberedOrigin = null; // e.g. "http://localhost:5174"
...
try {
  const kovaShimPath = path.join(__dirname, "kova-shim.js");
  kovaShimContent = fs.readFileSync(kovaShimPath, "utf-8");
  parentPort?.postMessage("[proxy-worker] kova-shim.js loaded.");
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read kova-shim.js: ${error.message}`,
  );
}
...
function injectHTML(buf) {
  ...
  // These are strings that were used since the first version of the kova shim.
  // If the kova shim is used from legacy apps which came pre-baked with the shim
  ...
  if (!legacyAppWithShim) {
    ...
    if (kovaShimContent) {
      scripts.push(`<script>${kovaShimContent}</script>`);
    } else {
      scripts.push(
        '<script>console.warn("[proxy-worker] kova shim was not injected.");</script>',
      );
    }
  }
