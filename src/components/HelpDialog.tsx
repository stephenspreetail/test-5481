...
import { ChatLogsData } from "@/types";
...
export function HelpDialog({ isOpen, onClose }: HelpDialogProps) {
  ...
  const handleReportBug = async () => {
    ...
    try {
      ...
      const debugInfo = await getClient().getSystemDebugInfo();
...
## System Information
- Kova Version: ${debugInfo.kovaVersion}
  ...
  const handleSubmitChatLogs = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "/TODO-API/upload-logs/generate-upload-url",
        {
          method: "POST",
          headers: {
      ...
      await getClient().uploadToSignedUrl(
        uploadUrl,
        "application/json",
        chatLogsJson,
      );
...
`;
          ...
          <div className="space-y-4 overflow-y-auto flex-grow">
            ...
            <div className="border rounded-md p-3">
              ...
              <div className="text-sm bg-slate-50 dark:bg-slate-900 rounded p-2 max-h-32 overflow-y-auto">
                <p>Kova Version: {chatLogsData.debugInfo.kovaVersion}</p>
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Need help with Kova?</DialogTitle>
        </DialogHeader>
            ...
            <Button
              ...
              onClick={() => {
                getClient().openExternalUrl(
                  "https://spreetail-engineering-playbook.tk.dev/docs",
                );
              }}
