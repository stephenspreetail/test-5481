...
import { VanillaMarkdownParser } from "@/components/chat/KovaMarkdownParser";
...
import type { SecurityFinding, SecurityReviewResult } from "@/types";
...
export const SecurityPanel = () => {
  ...
  const handleSaveRules = async () => {
    ...
    try {
      ...
      const { warning } = await client.editAppFile(
        selectedAppId,
        "SECURITY_RULES.md",
        rulesContent,
      );
  ...
  const handleRunSecurityReview = async () => {
    ...
    try {
      ...
      // Create a new chat
      const chatId = await getClient().createChat(selectedAppId);
