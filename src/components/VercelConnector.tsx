...
function UnconnectedVercelConnector({
  ...
  const loadAvailableProjects = async () => {
    ...
    try {
      const projects = await getClient().listVercelProjects();
  ...
  const handleSaveAccessToken = async (e: React.FormEvent) => {
    ...
    try {
      await getClient().saveVercelAccessToken({
  ...
  const checkProjectAvailability = useCallback(async (name: string) => {
    ...
    try {
      const result = await getClient().isVercelProjectAvailable({
        name,
      });
  ...
  const handleSetupProject = async (e: React.FormEvent) => {
    ...
    try {
      if (projectSetupMode === "create") {
        await getClient().createVercelProject({
          name: projectName,
          appId,
        });
      } else {
        await getClient().connectToExistingVercelProject({
          ...
          <div className="space-y-4">
            <div className="bg-teal-50 dark:bg-teal-900/20 border border-teal-200 dark:border-teal-800 rounded-md p-3">
              ...
              <div className="flex gap-2 mt-3">
                <Button
                  onClick={() => {
                    getClient().openExternalUrl("https://vercel.com/signup");
                  }}
                  variant="outline"
                  className="flex-1"
                >
