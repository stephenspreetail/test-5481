...
export function CapacitorControls({ appId }: CapacitorControlsProps) {
  ...
  const { data: isCapacitor, isLoading } = useQuery({
    ...
    queryFn: () => getClient().isCapacitor({ appId }),
  ...
  const syncAndOpenIosMutation = useMutation({
    mutationFn: async () => {
      ...
      // First sync
      await getClient().syncCapacitor({ appId });
      setIosStatus("opening");
      // Then open iOS
      await getClient().openIos({ appId });
    },
    onSuccess: () => {
      setIosStatus("idle");
  ...
  const syncAndOpenAndroidMutation = useMutation({
    mutationFn: async () => {
      ...
      // Then open Android
      await getClient().openAndroid({ appId });
    },
    onSuccess: () => {
            ...
            <Button
              ...
              onClick={() => {
                ...
                getClient().openExternalUrl(
                  "https://spreetail-engineering-playbook.tk.dev/docs/guides/mobile-app#troubleshooting",
                );
              }}
