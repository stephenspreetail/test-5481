...
export function useMcp() {
  ...
  const consentsQuery = useQuery<McpToolConsent[], Error>({
    ...
    queryFn: async () => {
      const client = getClient();
      const list = await client.getMcpToolConsents();
  ...
  const createServerMutation = useMutation({
    mutationFn: async (params: CreateMcpServer) => {
      const client = getClient();
      return client.createMcpServer(params);
    },
    onSuccess: async () => {
  ...
  const updateServerMutation = useMutation({
    mutationFn: async (params: McpServerUpdate) => {
      const client = getClient();
      return client.updateMcpServer(params);
    },
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["mcp", "servers"] });
      await queryClient.invalidateQueries({
  ...
  const setConsentMutation = useMutation({
    mutationFn: async (params: {
      ...
      const client = getClient();
      return client.setMcpToolConsent(params);
    },
    onSuccess: async () => {
