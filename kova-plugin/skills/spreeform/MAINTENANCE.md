...
> **Note:** The `/refresh-spreeform` skill is a project-level maintenance command in the kova repo. It's separate from the bundled `spreeform` skill because it's for maintaining this repo, not for distribution with the agent.

Or ask Claude Code directly:

> "Refresh the spreeform skill documentation by analyzing the latest @spreetail/spreeform package. Follow the instructions in skills/spreeform/MAINTENANCE.md"
...
Update these files in `skills/spreeform/`:
